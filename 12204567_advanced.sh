...
#Open two terminals. Do not forget to source the setup files and to set the required environment variables. On the first terminal run the publisher node, and the subscriber node on the other one. You should see that only the /async_topic messages are reaching the subscriber. 
...
#This will start publishing data on the chatter topic. As the bag-writing node receives this data, it will write it to the my_bag bag. If the my_bag directory already exists, you must first delete it before running the simple_bag_recorder node. 
#This is because rosbag2 will not overwrite existing bags by default, and so the destination directory cannot exist. Terminate both nodes. Then, in one terminal start the listener example node.         
ros2 run demo_nodes_cpp listener
...
#Open a terminal in the started VM and install the ROS 2 distribution you need by following the instructions in Ubuntu (Debian packages):
#Create a folder in the VM to use as a shared folder. In this example, the shared folder in the VM is /home/ubuntu/shared. 
...
#The environment variable WEBOTS_SHARED_FOLDER must always be set in order for the package to work properly in the VM. This variable specifies the location of the shared folder that is used to exchange data between the host machine and
#the virtual machine (VM) to the webots_ros2 package. The value to use for this variable should be in the format of <host shared folder>:<VM shared folder>, where <host shared folder> is the path to the shared folder on the host machine and <VM shared folder> is the path to the same shared folder on the VM.
...
#If not already set in ~/.bashrc, set WEBOTS_SHARED_FOLDER (see previous sections for details). Be sure to change the paths according to the location of your respective directories.
export WEBOTS_SHARED_FOLDER=/Users/username/shared:/home/ubuntu/shared

#Use the ROS 2 launch command to start demo packages (e.g. webots_ros2_universal_robot).
ros2 launch webots_ros2_universal_robot multirobot_launch.py

#n the pre-configured images, WEBOTS_SHARED_FOLDER is already set in ~/.bashrc. You will need to update it to use the correct path for the host folder:
export WEBOTS_SHARED_FOLDER=/Users/username/shared:/home/ubuntu/shared  
...
#As mentioned in Setting up a robot simulation (Basic), webots_ros2_driver contains plugins to interface most of Webots devices with ROS 2 directly. These plugins can be loaded using the <device> 
#tag in the URDF file of the robot. The reference attribute should match the Webots device name parameter. The list of all existing interfaces and the corresponding parameters can be found on the devices reference page. 
...
#The node is imported using the importMFNodeFromString(nodeString) API function.
...
#If the node is indeed in the imported list, it is removed with the remove() API method.
...
#The reset handler allows you to restart specific nodes or perform additional actions when the reset button in Webots is pressed. This can be useful for scenarios where you need to reset the state of your simulation or restart specific components without completely restarting the complete ROS system.
